...
AI_ANALYSIS_SCHEMA_VERSION = "scenario-v4"
...
class AIAnalyzer:
    ...
    def _build_prompt(
        ...
        return (
            ...
            "请像一线合约交易台的风控分析师在给同事讲盘一样，输出一段连贯的专业观点，"
            "Telegram/网页纯文本：不用 Markdown 标记，不用编号、小标题和列表符号，"
            "写成 2-4 个自然段，段落之间空一行：\n"
            "- 开头一两句直接给核心判断：这波更像什么（延续、反抽、突破确认、假突破、踩踏、分歧等待等），"
            "方向倾向和把握程度，必须结合后效统计和多周期共振来定调。\n"
            "- 中间围绕这组数据里最突出的驱动或矛盾展开推理，不要面面俱到；"
            "判断必须落到具体数字上：结构支撑/压力、VWAP、盘口墙、量能、OI、爆仓中至少引用三个，"
            "用来支撑推理，而不是机械复读字段。\n"
            "- 必须写清失效边界：哪个具体价位被破坏或哪类行为出现后，当前判断作废、应当认错。\n"
            "- 结尾点出接下来最该盯的 1-2 个信号（如二次放量、回踩是否缩量、OI 是否跟随、关键位争夺），"
            "不给绝对开仓指令。\n"
            f"{period_focus}"
            "如果样本可信度为 low 或样本不足，必须明确降低结论强度。"
            "如果爆仓数据状态不可用，不要把爆仓金额为0解读为没有强平压力。"
            "全文 250-450 字：字数不用多，但核心判断、失效边界和观察信号缺一不可，"
            "语言自然、有判断、有取舍，像人讲盘，不要像机器人填表。"
        )
